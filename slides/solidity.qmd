...
- The main reference for this slide set is the [Solidity language documentation](https://docs.soliditylang.org/en/latest/index.html)
  - URL: [https://docs.soliditylang.org/en/latest/index.html](https://docs.soliditylang.org/en/latest/index.html)
- This section is the [Types section therein](https://docs.soliditylang.org/en/latest/types.html)
  - [https://docs.soliditylang.org/en/latest/types.html](https://docs.soliditylang.org/en/latest/types.html)
...
```
string s = "CS" '4790'; // equals "CS4790"
```
...
- See them all in the [Solidity types reference](https://docs.soliditylang.org/en/latest/types.html)
  - URL is [https://docs.soliditylang.org/en/latest/types.html](https://docs.soliditylang.org/en/latest/types.html)
...
- [Ethereum developer resources](https://ethereum.org/en/developers/)
  - URL: [https://ethereum.org/en/developers/](https://ethereum.org/en/developers/)
...
- [Remix documentation](https://remix-ide.readthedocs.io/en/latest/) (the Solidity IDE)
  - URL: [https://remix-ide.readthedocs.io/en/latest/](https://remix-ide.readthedocs.io/en/latest/)
...
// SPDX-License-Identifier: glurgle
```
...
- This will only compile with version 0.8.33 or later
...
- `selfdesctruct()`
  ...
  - Currently deprecated, to be removed "soon"
...
- A contract can have:
  ...
  - A few other special-purpose functions we won't see here
  - Modifiers
...
## Solidity reference
- See the Solidity cheatsheet online at [https://docs.soliditylang.org/en/latest/cheatsheet.html](https://docs.soliditylang.org/en/latest/cheatsheet.html)
...
- Compiled bytecode
  - [Normal](../hws/dappintro/Poll-normal.bin.txt) (5,607 bytes)
    - Contract @ 0x6C58b0ed405FD557c4F9a8Aa18525871532806F6 (spring 2026)
  - [Optimized](../hws/dappintro/Poll-optimized.bin.txt) (3,011 bytes)
    - Contract @ 0xd2Cc8a43B9987aDE3Ae74Bb33030d3EC03E5c1b7 (spring 2026)
...
- The next column of slides (to the right) is not
...
- On the blockchain explorer is the relevant contract address (and other information)

## Blockchain Explorer
- Look at the Blockchain Explorer
  - There are links at the top to the Debts contract and a few TXNs
  - Also to the PHP page that can view information about the Debts contract
- For the transactions:
  - The first one deployed it: notice the very large `input` field, and there is no `inputDecode` field
  - The second one creates an alias; notice the `functionCall` and `inputDecode` fields
  - The third one reverted; notice the `status` field that lists the reversion reason
...
- On the blockchain explorer, at the top, are a few pieces of information we will need:
...
Consider the Debug.sol library:
...
/* Debug.sol
 *
 * This library has various functions used for debugging.  
 * By calling the various debug*() functions, an event is 
 * fired, and the blockchain explorer will display the 
 * parameters for that event.  Thus, one can use it as a 
 * stand-in for a print statement.
 */
...
library Debug {
    ...
    // These functions will emit the event with the same 
    // argument type
    function debugUint(uint s) public {
      emit debugEventUint(s);
    }

    function debugInt(int s) public {
      emit debugEventInt(s);
    }

    function debugAddress(address s) public {
      emit debugEventAddress(s);
    }

    function debugBytes32(bytes32 s) public {
      emit debugEventBytes32(s);
    }

    function debugString(string memory s) public {
      emit debugEventString(bytesToBytes32(bytes(s)));
    }


    // These are the events that are fired from the above debug 
    // functions
    ...
    // Internal utility function to convert an array of bytes1 
    // into a bytes32 value
    function arrayToBytes32(bytes1[32] memory arr) internal pure 
                                        returns (bytes32 result) {
        for (uint i = 0; i < 32; i++)
            result |= bytes32(arr[i]) >> (i * 8);
    }

    // Internal utility function to convert a variable-sized bytes 
    // into bytes32; only the first 32 characters are converted, 
    // and it is right-padded with 0 bytes if the parameter is less 
    // than 32 bytes in size
    function bytesToBytes32(bytes memory data) internal pure 
                                    returns (bytes32 result) {
        bytes1[32] memory a;
...
![](images/solidity-event-debugging.png){alt='solidity debugging icon'}

This is *new*, so be patient if it is a bit buggy at first...
