...
## [Advanced Encryption Standard (AES)]{.r-fit-text}
...
## [Public key cryptography challenges]{.r-fit-text}
...
- The only ones that were not were:
    ...
    3. Finding the tangent of an elliptic curve at a point
- We can perform these two with basic arithmetic by:
    1. Being [given an initial point on the curve](#/secp256k1-curve-base-point)
    2. Taking a [formulaic shortcut to find the intersection of a line and curve](#/elliptical-point-addition-1)
    2. Taking [another, different, formulaic shortcut](#/elliptical-point-addition-2)
...
::: {.right-float-img}
![](https://upload.wikimedia.org/wikipedia/commons/a/a4/Clock_face_one_hand.png){fig-alt="an image of a clock"}
:::
...
- An integer [finite field](https://en.wikipedia.org/wiki/Finite_field):
  - Is defined by a (typically prime) value $p$
...
- Exponentiation: easy -- just exponentiate and mod by $p$
  - $3^3 = 27 \mod 17 = 10$
  - There are more efficient ways to do this, discussed later
...
## Finite Field Mult Inverse
- To find the multiplicative inverse in $Z_p$:
  - $1/a = a^{-1} = a^{p-2} \mod p$
- Example: $a=13$, $p=17$
...
- The net result is that given a value $a$, we can quickly and easily compute $a^{-1}$
...
- Given a point $P$ on a curve, you can keep elliptically adding it to itself to get $R=k \otimes P$
...
## secp256k1
![](images/encryption/secp256k1/secp256k1-a.svg){fig-alt="an image of the secp256k1 curve"}
...
<aside data-markdown class="notes">
- because we want to find the multiplicative inverse of $k$, in order to use Fermat's little theorem, $o$ must be prime
</aside>
...
::: {.stretch}
![](https://imgs.xkcd.com/comics/random_number.png){title="RFC 1149.5 specifies 4 as the standard IEEE-vetted random number." style='border:10px solid white' fig-alt='xkcd comic on randomness'}
:::
::: {.r-stack}
...
## [Why libc is not cryptographically secure]{.r-fit-text}
...
- Examples:
  ...
  - Interpret the numbers as 5 card stud poker hands, and see if the distribution of hands is the expected distribution
  - And many others...


## Security
::: {.stretch}
![](https://imgs.xkcd.com/comics/security.png){title="Actual actual reality: nobody cares about his secrets.  (Also, I would be hard-pressed to find that wrench for $5.)" style='border:10px solid white' alt='xkcd comic on security'}
:::
::: {.r-stack}
...
## [Ensuring the download is correct...]{.r-fit-text}
...
- There are more files possibilities of 129 bits (or 513 bits) than there are possible hashes
...
- NIST (National Institute for Standards and Technology) had an open solicitation / competition for the algorithm
...
## [Calling SHA-256 from the command line]{.r-fit-text}
...
## [Openssl to generate & view ECDSA keys]{.r-fit-text}
