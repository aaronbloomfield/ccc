...
## Cryptocurrencies
- There are 10,626 existing cryptocurrencies (that have value) <!-- change each semester -->
  - As per [investing.com](https://www.investing.com/crypto/currencies) (as of Jan 24, 2025)
...
- The data in this slide set was all updated for the current semester (as of Jan 13, 2025) <!-- change each semester -->
...
- The current blockchain (as of Jan 13, 2025) is  826,343 blocks (aka 'height'; [source](https://bitinfocharts.com/))
  - About 530 Gb in size ([source](https://www.blockchain.com/charts/blocks-size))
...
- The current target ($ct$) is 000000000000000000028c63e000000 000000000000000000000000000000000
<!-- formula: ct=244088142064384875447319840151416852295254855034623487 -->
  - As of Jan 13, 2025 and derived from [here](https://blockstream.info)
  <!-- change each semester -->
  - The new target is harder than the old: specifically $ft/ct$ = 110.45 *trillion*
...
- The current target is 110.45 trillion times harder
  - A random nonce now has a 1 in $4.74 \times 10^{23}$ chance of success
...
- Consider a 1 in $4.74 \times 10^{23}$ chance of success
- To generate $4.74 \times 10^{23}$ hashes at 1 billion (1 Ghash) per second on a fast graphics card, this will take 18.8 *million years*
  - A CPU is 40 times longer: about 750 *million years*
...
- This would cost about 9.9 billion dollars <!-- change each semester; formula: elecost=gpuyears*400/1000*24*365.25*0.15 -->
...
## Reward per block
- The current reward (as of Jan 13, 2025) is 3.125 BTC
  <!-- change each semester -->
...
- At a price of about $94k per block ([source](https://www.coindesk.com/price/bitcoin)), 3.225 BTC is about $303k for a properly signed block   <!-- change each semester --> <!-- formula: reward=303000 -->
  - But if it costs almost 10 billion dollars in electricity, perhaps it's not all that worth it
...
## Hashes per second
From [blockchain.com](https://www.blockchain.com/charts/hash-rate), and for the entire Bitcoin network: 764 exahashes per second (exa = $10^{18}$)
<!-- change each semester -->
<!-- formula: hashrate=534*10^18 -->
...
- Consider the [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7), which costs `$`1,700 (as of Jan 13, 2025)
...
<!-- change each semester -->
- We know that:
  - The current hash rate is about 534 EH/s
...
- 764 exa / 120 tera = $764 \cdot 10^{18} / 120 \cdot 10^{12} = 4.45 \cdot 10^{6}$ or about 4.5 million <!-- formula: blockreward=hashrate/minerrate -->
- Thus, you would expect to get one 4.5-millionth of a block's reward in a pooled setup <!-- formula: profitperblock=reward/blockreward -->
- At `$`303k per block, that's about 6.8 *cents* per block
...
## [DragonX AntMiner S19](https://www.amazon.com/dp/B08ZNMGKZ1) analysis
<!-- change each semester -->
- At `$`303 per block, that's about 6.8 cents per block
  - Which is $9.80 per day <!-- formula: profitperday=profitperblock*24*6 -->
    - This assumes 10 minute blocks; it's a less with 12m6s blocks
  - You will make back your initial investment of `$`1,200 (plus tax) 192 days <!-- formula: payoffdays=1200/profitperday -->
    - That's 6.5 months
- This assumes that the numbers don't change!
  - And that you aren't paying for electricity
  - And we know the reward per block will go down in late April of 2024...
...
## [DragonX AntMiner S19](https://www.amazon.com/dp/B08ZNMGKZ1) analysis
<!-- change each semester -->
- You will make $9.80 in BTC per day
...
- Stated power usage: 2,760 W or 2.76 kW
  ...
  - At Charlottesville's rate of 15&cent; per kWh, that's `$`9.93 per day
    - About `$`298 per month
- So your total daily profit is: `$`9.80 - `$`9.93 or
  - *Negative* `$`0.13 cents per day!
...
- When done on a large scale
  - One can make deals with power companies, locate where power is cheaper, buy miners cheaper, avoid import tariffs, etc.
