...
![](images/tokens/dollar-bill.webp){fig-alt='dollar bill image'}
:::
...
- Representative currency: one that is backed up by a valuable "thing", such as gold
  - When the US dollar was on the "[gold standard](https://en.wikipedia.org/wiki/Gold_standard)"
...
## Consider the AaronDollar&trade;
::: {.no-white-img-background .max-image-height-300 .noboxshadow}
![](images/overview/aaron-dollar.png){fig-alt='AaronDollar(TM) image'}
:::
...
- Some examples:
  ...
  - <img src="https://upload.wikimedia.org/wikipedia/commons/e/e3/Moneda_1_Bol%C3%ADvar_2021.png" class="right-float-img max-image-height-300 no-white-img-background"> Due to the [crisis in Venezuela](https://en.wikipedia.org/wiki/Crisis_in_Venezuela), the Venezuelan [bolivar soberano](https://en.wikipedia.org/wiki/Venezuelan_bol%C3%ADvar) (VEB), and its predecessor the bolivar fuerte, are essentially worthless
...
## Cryptocurrencies
- There are 8,992 existing cryptocurrencies (that have value) <!-- change each semester -->
  - As per [investing.com](https://www.investing.com/crypto/currencies) (as of Jan 12, 2026)
...
::: {.cclogosubtitle}
![](images/logos/btc-coin-symbol.svg){fig-alt="BTC logo"}
:::


## A note about numbers...
<a href='https://commons.wikimedia.org/wiki/File:One_bitcoin_sitting_atop_bundles_of_US_$100_notes.png'><img src='images/overview/bitcoin-coin.webp' class='right-float-img-nb-300'></a>

- The data in this slide set was all updated for the current semester (as of Jan 12, 2026) <!-- change each semester -->
...
::: {.right-float-img-nb-400}
![](https://upload.wikimedia.org/wikipedia/commons/7/77/Satoshi_Nakamoto.jpg){fig-alt='picture of satoshi nakamoto statue'}
:::

- The first viable cryptocurrency,sss it was created in 2008 by Satoshi Nakamoto
...
## Bitcoin price
From [blockchain.com](https://blockchain.com); BTC peaked at $126,032 in October 2025 ([source](https://www.blockchain.com/charts/market-price?timespan=all))
![](images/timely/btc-price.webp){fig-alt="BTC price graph"}
...
::: {.right-float-img-nb-400}
![](https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/Czech_Wallet.jpg/800px-Czech_Wallet.jpg){fig-alt='image of a billfold wallet'}
:::
...
- The current blockchain (as of Jan 12, 2026) is  932,038 blocks (aka 'height') ([source](https://bitinfocharts.com/))
  - About 696 Gb in size ([source](https://www.blockchain.com/charts/blocks-size))
...
## Simple Blockchain
::: {.right-float-img}
![](images/overview/graphs/overview.dot.1.svg){fig-alt='simple image of a blockchain'}
:::
...
## Target
::: {.right-float-img-nb-300}
![](https://upload.wikimedia.org/wikipedia/commons/d/dc/WA_archery_target_with_arrows.jpg){fig-alt='picture of an archery target'}
:::
...
- The current target ($ct$) is 00000000000000000001ebf2 0000000000000000000000000000000000000000
<!-- formula: ct=184058593202179251712735660462623250929428832597311488 -->
  - As of Jan 12, 2026 and derived from the nbits value of 0x1701EBF2 from the latest block
  <!-- change each semester --><!-- nbits conversion formula at mining.html#/mining-nbits -->
  - The new target is harder than the old: specifically $ft/ct$ = 146.47 *trillion*
...
- The current target is 146.47 trillion times harder
  - A random nonce now has a 1 in $6.29 \times 10^{23}$ chance of success
...
::: {.right-float-img-nb-400}
![](https://upload.wikimedia.org/wikipedia/commons/4/48/Gigabyte_GTX_970_Windforce.jpg){fig-alt='picture of a video card'}
:::
- A typical CPU can do maybe 10 Mhash/sec (million hashes per second)
  - A very fast CPU today could do about 25 Mhash/sec
  - A [$1,500 video card](https://www.amazon.com/gp/product/B07GHXQTGZ/) today could do about 1 Ghash/sec (estimate based on [this](https://en.bitcoin.it/wiki/Non-specialized_hardware_comparison#Nvidia) and [this](https://gpu.userbenchmark.com/Compare/Nvidia-RTX-2080-vs-Nvidia-GeForce-GTX-590-SLI-Disabled/4026vsm8041))
    - 1 Ghash/sec is 40 times faster than a CPU
- Consider a 1 in $6.29 \times 10^{23}$ chance of success
- To generate $6.29 \times 10^{23}$ hashes at 1 billion (1 Ghash) per second on a fast graphics card, this will take 24.9 *million years*
  - A CPU is 40 times longer: about 1 *billion years*
  <!-- change each semester; formula: gpuyears=chance/(365.25*24*60*60*800*10^6) 
  formula: cpuyears = 40*gpuyears -->



## Electricity usage
::: {.right-float-img-nb-300}
![](https://upload.wikimedia.org/wikipedia/commons/6/6b/Electricity_pylons2.jpg){fig-alt='picture of electricity poles'}
:::
...
- Electricity in Charlottesville costs 15 cents per kW/h (as per [here](https://www.electricitylocal.com/states/virginia/charlottesville/))
- This would cost about 13.1 billion dollars <!-- change each semester; formula: elecost=gpuyears*400/1000*24*365.25*0.15 -->
...
![](images/timely/btc-difficulty-all-time.webp){fig-alt="BTC difficulty, all time"}{fig-alt='BTC difficulty graph'}
<!-- change each semester -->
...
![](images/timely/btc-difficulty-last-3-years.webp){fig-alt="BTC difficulty, last 2 years"}{fig-alt='BTC difficulty graph'}
<!-- change each semester -->
...
## Reward per block
- The current reward (as of Jan 12, 2026) is 3.125 BTC
  <!-- change each semester -->
  - Plus transaction fees of around 0.1 BTC or so
  - The estimated date when it drops to 1.56 BTC is mid April 2028 ([source](https://www.bitcoinblockhalf.com/))
- At a price of about $91k per block ([source](https://www.coindesk.com/price/bitcoin)), 3.225 BTC is about $293k for a properly signed block   <!-- change each semester --> <!-- formula: reward=303000 -->
  - But if it costs almost 13 billion dollars in electricity, perhaps it's not all that worth it
...
## Pooled mining
- If it would take your high-end GPU 10 billion years to compute a block, and blocks need to be computed every 10 minutes, then you aren't going to get very far on your own
...
## Hashes per second
From [blockchain.com](https://www.blockchain.com/charts/hash-rate), and for the entire Bitcoin network: 1.06 zetahashes per second (zeta = $10^{21}$)
<!-- change each semester -->
<!-- formula: hashrate=1.06*10^21 -->
![](images/timely/btc-hashes-per-sec.webp){fig-alt="hashes per second"}{fig-alt='hashes per second graph'}



## Mining hardware
::: {.right-float-img-nb-400}
![](https://m.media-amazon.com/images/I/61WsmRwaOaL._AC_SL1000_.jpg){fig-alt='picture of an ASIC miner'}
:::
...
- Consider the [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7), which costs `$`1,000 (as of Jan 12, 2026)<!-- change each semester -->
  - It can compute 120 Thash/sec (tera), or $120 \ast 10^{12}=1.2 \ast 10^{14}$ hash/sec
...
## [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7) analysis
<!-- change each semester -->
- We know that:
  - The current hash rate is about 1.06 ZH/s
  - Blocks take about 10 minutes to process (actually about 9m 56s right now ([source](https://bitinfocharts.com/)))
  - The [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7) claims to hash 120 TH/s <!-- formula: minerrate=120*10^12 -->
- 1.06 zeta / 120 tera = $1.06 \cdot 10^{21} / 120 \cdot 10^{12} = 4.45 \cdot 10^{6}$ or about 8.83 million <!-- formula: blockreward=hashrate/minerrate -->
- Thus, you would expect to get one 8.8-millionth of a block's reward in a pooled setup <!-- formula: profitperblock=reward/blockreward -->
- At `$`293k per block, that's about 3.3 *cents* per block
...
## [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7) analysis
<!-- change each semester -->
- At `$`293 per block, that's about 3.3 cents per block
  - Which is $4.78 per day <!-- formula: profitperday=profitperblock*24*6 -->
    - This assumes 10 minute blocks; it's a bit more with 9m 56s blocks
  - You will make back your initial investment of `$`1,000 (plus tax) 250 days <!-- formula: payoffdays=1200/profitperday -->
    - That's 8.3 months
- This assumes that the numbers don't change!
  - And that you aren't paying for electricity
  - And we know the reward per block will go down in late April of 2028...
- There is also [one available](https://www.amazon.com/dp/B0DXZ6874R/) that mines twice as much but costs 3.5 times as much, and uses only 34% more power



## [DragonX AntMiner S19k](https://www.amazon.com/dp/B0CLVJ1KH7) analysis
<!-- change each semester -->
- You will make $4.78 in BTC per day
- But... electricity is not free
- Stated power usage: 2,760 W or 2.76 kW
  - Over 24 hours that's $24 * 2.76 = 66$ kWh
  - At Charlottesville's rate of 15&cent; per kWh, that's `$`9.93 per day
    - About `$`298 per month
- So your total daily profit is: `$`4.78 - `$`9.93 or
  - *Negative* `$`5.15 cents per day!
...
- When done on a large scale
  - One can make deals with power companies, relocate where power is cheaper, buy miners cheaper, avoid import tariffs, etc.
...
- Graphs: see [here](images/timely/readme.html)
