...
<img src="../slides/images/logos/btc-coin-symbol.svg" class="cclogosubtitle" alt='BTC symbol'>
<img src="../slides/images/logos/eth-coin-symbol.svg" class="cclogosubtitle" alt='ETH symbol'>
<img src="../slides/images/logos/rvn-coin-symbol.svg" class="cclogosubtitle" alt='RVN symbol'>


## Disclaimer
- All the numbers shown here were accurate and current as of the time this slide set was updated
  - February 12, 2025   <!-- change each semester -->
...
- [![btc logo](../slides/images/logos/btc-coin-symbol.svg){alt='btc logo'}](https://coinmarketcap.com/currencies/bitcoin/){class="cclogo-slide"}
 Bitcoin: we all know this
  - Hash algorithm: double SHA-256
- [![rvn logo](../slides/images/logos/rvn-coin-symbol.svg){alt='rvn logo'}](https://coinmarketcap.com/currencies/ravencoin/){class="cclogo-slide"}
 Ravencoin
  - A fork of Bitcoin, so works similarly; focuses on tokens
  - Designed to be ASIC resistant
  - Hash algorithm: kawpow (16 different rotating algorithms)
- [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide"}
 Ethereum
...
- In [Ravencoin block 3,714,773](https://rvnblockexplorer.com/block/3028834):  000000000000 9847addedabc03b320a9545175d969c7cf23dc9b862f301453cc
    - 12 leading hex zeros, 48 leading bits
- In [Bitcoin block 883,513](https://bitcoinblockexplorers.com/block/813369):  00000000000000000000 d8dba3009cc023e03220d0ec6e5e3285e15ba0d9f0dd <!-- change each semester -->
...
- Thus...
<ul>
  <li class='fragment'> RVN example: $\frac{1}{2^{48}}$, which is 1 in 281 trillion ($2.81 \ast 10^{14}$), would have 48 leading 0 bits </li>
  <li class='fragment'> BTC example: $\frac{1}{2^{80}}$, which is 1 in 1.2 septillion ($1.2 \ast 10^{24}$), would have 80 leading 0 bits </li>
...
<a href='https://twitter.com/100trillionUSD/status/1081217034485149697'><img src='images/mining/nonce-tweet.webp' style="width:40%;border-radius:10px" alt='diagram of bitcoin nonce distribution'></a>

- Consider [The Mystery Of The Bitcoin Nonce Pattern](https://blog.bitmex.com/the-mystery-of-the-bitcoin-nonce-pattern/)
  - And the [full image](images/mining/nonce-tweet-graph.webp) ([remote](https://pbs.twimg.com/media/DwFAIi8WoAAyUwq?format=jpg&name=large))
- Early blocks (left-most) were Satoshi experimenting
- Possible reasons:
  - Selection bias ([source](https://bitcoin.stackexchange.com/questions/36860/bitcoin-and-other-crypto-coins-nonce-loop))
  - How ASICs work ([source](https://blog.bitmex.com/the-mystery-of-the-bitcoin-nonce-pattern/))
...
- An nbits field of 0x1b00a908 thus gets us a *target* of 0x000000000000a908000000000000000 000000000000000000000000000000000
  - This is from [RVN block 1,982,364](https://rvnblockexplorer.com/block/1982364) (from October 2021)
...
- The first RVN target ([RVN block 1](https://rvnblockexplorer.com/block/1)) was 0x1d00ffff = 0x00000000ffff00000000000000000000 00000000000000000000000000000000
...
- You can see the current [RVN network difficulty](https://www.coinwarz.com/mining/ravencoin/difficulty-chart)
...
- If a share requires 42 leading zero bits, then it would report a share difficulty of $2^{42} = 4.4$ T
  ...
  - The target for [RVN block 1,982,364](https://rvnblockexplorer.com/block/1982364) needed 48 leading zero bits
    - $s_d=2^{48} = 281.5$ T
...
- Any *good* nonce is reported to the mining pool, even if it is not enough to solve the block
  - Consider the [2miners RVN mining pool](https://rvn.2miners.com/), which has *pdiff* (pool mining difficulty) of 4.295 G
...
## [How much is mining killing the Earth?]{.r-fit-text}

- Consider the [Avalon A1466 miner](https://www.canaan.io/product/avalon-made-a1466-150t)
  ...
  - At Charlottesville's electric rates (15&cent; per kWh) that's $11.67 per day
- [This site](https://www.blockchain.com/charts/hash-rate) estimates that the current world-wide BTC hash rate is 804 million Th/s
  - If it were only Avalon A1466 miners, then we need 5.37 million such miners, at 150 Th/s, to get 804 million Th/s
  - That's 17.4 GW ($10^{12}$W), or 416 GWh per day, or about 152,000 GWh (152 TWh) per year <!-- change each semester -->



## Death to the Environment!
[![](https://upload.wikimedia.org/wikipedia/commons/thumb/a/a4/Alfred_Palmer_Smokestacks.jpg/1280px-Alfred_Palmer_Smokestacks.jpg)](https://commons.wikimedia.org/wiki/File:Alfred_Palmer_Smokestacks.jpg){class="right-float-img-600 no-border" alt='picture of smoke stacks'}

- The lower-bound estimate from the last slide is that BTC mining uses 17.4 GW ($10^{12}$W), or 416 GWh per day, or 152 TWh per year  <!-- change each semester -->
  - A [2019 estimate](https://www.sciencedirect.com/science/article/pii/S2542435119302557) was at 46 TWh per year, but the [hash rate](https://www.blockchain.com/charts/hash-rate?timespan=all) has increased significantly since then
- That is more than 171 of the world's 218 countries ([source](https://en.wikipedia.org/wiki/List_of_countries_by_electricity_consumption))
- That is the equivalent to 2% of the total US electrical usage per year ([source](https://en.wikipedia.org/wiki/List_of_countries_by_electricity_consumption))
...
- So perhaps the actual amount is twice our initial estimate?
  - That is the equivalent to 4% of the total US electrical usage per year ([source](https://en.wikipedia.org/wiki/List_of_countries_by_electricity_consumption))
  - That's 166 TWh per year
  - But that's a total guess, though!


## [Can mining not kill the environment?]{.r-fit-text}
...
- In 2021, the world used 25,343 TWh of electricity ([source](https://en.wikipedia.org/wiki/List_of_countries_by_electricity_consumption))
...
- That is 0.63% of the worldwide total energy usage
  - 0.32% for Bitcoin only <!-- change each semester -->



## Actual usage
- Consider: [Bitcoin Mining Carbon Emissions at 'Inconsequential' Levels: CoinShares](https://decrypt.co/91824/bitcoin-mining-carbon-emissions-inconsequential-levels-coinshares)
  - They estimate that bitcoin mining uses 0.08% of worldwide electricity
- Take that with a grain of salt, though!
  - A cryptocurrency company, that makes money off of mining, saying it's fine for the environment is like a cigarette company saying smoking is not unhealthy
...
- Reasons:
  ...
  - Heat can be re-used for other purposes
    - See [this Google images search](https://www.google.com/search?q=bitcoin+miner+heat+re-use&tbm=isch)
    - Using the heat for other purposes that you would have paid for makes it more profitable




## BTC mining in the US
![](https://upload.wikimedia.org/wikipedia/commons/3/35/Texas_flag_map.svg){class="right-float-img" alt='image of the Texas flag'}

- [Texas is the go-to place to mine BTC](https://www.washingtonpost.com/technology/2021/07/08/bitcoin-mining-texas-electricity/)
  - Cheap energy and a business-friendly environment
- The miners [turn off the mining when demand spikes](https://www.washingtonpost.com/climate-solutions/2022/07/15/bitcoin-crypto-texas-heat-wave/)
  - And they [*get paid to do so*](https://finance.yahoo.com/news/texas-bitcoin-miners-getting-paid-064946406.html) ([original source](https://fortune.com/2022/07/12/texas-bitcoin-miners-paid-shut-down-power-grid-electricity-blackout-heat/))
  <!-- - The cost of this is pushed off to consumers in other ways: [low social services](https://finance.yahoo.com/news/10-states-spend-most-least-090000541.html) (6th worst), [low quality of life](https://www.worldatlas.com/articles/the-10-us-states-with-the-worst-quality-of-life.html) (5th worst), [highest overall taxes](https://worldpopulationreview.com/state-rankings/highest-taxed-states) (7th worst), etc. -->
...
<img src="../slides/images/logos/eth-coin-symbol.svg" class="cclogosubtitle" alt='ETH symbol'>
<img src="../slides/images/logos/fil-coin-symbol.svg" class="cclogosubtitle" alt='FIL symbol'>
<img src="../slides/images/logos/neox-coin-symbol.svg" class="cclogosubtitle" alt='NEOX symbol'>
<img src="../slides/images/logos/ppc-coin-symbol.svg" class="cclogosubtitle" alt='PPC symbol'>
<img src="../slides/images/logos/rvn-coin-symbol.svg" class="cclogosubtitle" alt='RVN symbol'>
<img src="../slides/images/logos/sol-coin-symbol.svg" class="cclogosubtitle" alt='SOL symbol'>
<img src="../slides/images/logos/storj-coin-symbol.svg" class="cclogosubtitle" alt='STORJ symbol'>
<img src="../slides/images/logos/xlm-coin-symbol.svg" class="cclogosubtitle" alt='XLM symbol'>
<img src="../slides/images/logos/xpm-coin-symbol.svg" class="cclogosubtitle" alt='XPM symbol'>
...
- [![xpm logo](../slides/images/logos/xpm-coin-symbol.svg){alt='xpm logo'}](https://coinmarketcap.com/currencies/primecoin/){class="cclogo-slide"} Example: Primecoin, launched in 2013 <br clear='all'>
  - Purpose: generate primes for mathematical and scientific study
  - [Whitepaper](https://primecoin.io/bin/primecoin-paper.pdf)
...
<img src="https://static.canaan.io/prod/u_file/2306/21/products/20230621-132459A13-5a80.jpeg?x-oss-process=image/quality,q_80/resize,m_lfit,h_500,w_500" style="padding-left:0;padding-right:0" class="right-float-img" alt='picture of an ASIC miner'>
...
- [![rvn logo](../slides/images/logos/rvn-coin-symbol.svg){alt='rvn logo'}](https://coinmarketcap.com/currencies/ravencoin/){class="cclogo-slide"} Consider Ravencoin, which aims for ASIC-resistant mining <br clear='all'>
...
## [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide"} Ethereum and PoS
- Originally Ethereum was proof-of-work (hash algorithm: Ethash)
- It had a [difficulty bomb](https://docs.ethhub.io/questions-about-ethereum/what-is-the-difficulty-bomb/), aka "Ice Age"
  - At certain block numbers, the difficulty increases significantly
  - See [this graph](https://i.imgur.com/ac1xV3l.png) from the above link
...
## [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide"} Ethereum's move to PoS
- Phase 0: December 2020: create the "Beacon Chain", a PoS blockchain
- Phase 1: September 15, 2022: change Ethereum from PoW to PoS
  - Last PoW block: [15,537,393](https://etherscan.io/block/15537393)
  - First PoS block: [15,537,394](https://etherscan.io/block/15537394)
- Phase 2: sometime in ~~2023~~ ~~2024~~ 2025?: allow shards
  - We'll see [sharding in the scalability slide set](scalability.html#/sharding) 



## [![ppc logo](../slides/images/logos/ppc-coin-symbol.svg){alt='ppc logo'}](https://coinmarketcap.com/currencies/peercoin/){class="cclogo-slide"} Peercoin
- First implementation of PoS, from 2012
  - [Whitepaper](https://whitepaper.io/document/139/peercoin-whitepaper), [source code on github](https://github.com/peercoin/peercoin)
...
- Weakness: buy a lot of Peercoin, and wait a long time
  ...
  - Then execute a [51% attack](bitcoin.html#/attacks) (we'll see this shortly)
...
## Proof-of-History
- In ![sol logo](../slides/images/logos/sol-coin-symbol.svg#cclogo){class="cclogo-slide" alt='SOL logo'} Solana ([SOL](https://coinmarketcap.com/currencies/solana/)) the block verification combines PoS and proof of history ([Solana's whitepaper](https://solana.com/solana-whitepaper.pdf))
...
- With ![btc logo](../slides/images/logos/btc-coin-symbol.svg#cclogo){class="cclogo-slide" alt='BTC logo'} Bitcoin, we have to compute the Merkle tree hash
  - Given $n$ transactions, it takes $\Theta(n)$ steps to build the tree
  - Given infinite computing power, it takes $\Theta(\log n)$ steps
- In ![sol logo](../slides/images/logos/sol-coin-symbol.svg#cclogo){alt='SOL logo'} Solana, each transaction can be verified in parallel
...
- [![storj logo](../slides/images/logos/storj-coin-symbol.svg){alt='storj logo'}](https://coinmarketcap.com/currencies/storj/){class="cclogo-slide"} Example: Storj
...
- [![fil logo](../slides/images/logos/fil-coin-symbol.svg){alt='fil logo'}](https://coinmarketcap.com/currencies/filecoin/){class="cclogo-slide"} Example: Filecoin



## [![neox logo](../slides/images/logos/neox-coin-symbol.svg){alt='neox logo'}](https://coinmarketcap.com/currencies/neoxa/){class="cclogo-slide"} Neoxa: "Proof-of-Game"
- This is *not* a block certification method!
  - They just wanted to use the "proof-of-X" moniker
- [Neoxa's whitepaper](https://www.neoxa.net/whitepaper)
- Intent is to reward video game players for actions taken in the game itself
  - See the table on [whitepaper page 3](https://www.neoxa.net/whitepaper)
...
- Possibilities:
  ...
  - Does not require proofs, such as [![xlm logo](../slides/images/logos/xlm-coin-symbol.svg){alt='xlm logo'}](https://coinmarketcap.com/currencies/stellar/){class="cclogo-slide"}
 Stellar Lumens (XLM)
...
<img src="images/mining/dag/dag.dot.1.svg" style="height:20px" alt='image of a DAG; each one is one more level than the last'>



## The first block mined
<img src="images/mining/dag/dag.dot.2.svg" alt='image of a DAG; each one is one more level than the last'>
Block 0 is on the top, block 1 is on the bottom



## Two blocks mined
<img src="images/mining/dag/dag.dot.3.svg" alt='image of a DAG; each one is one more level than the last'>
We've just mined block 2



## A transaction
<img src="images/mining/dag/dag.dot.4.svg" alt='image of a DAG; each one is one more level than the last'>


## 
<img src="images/mining/dag/dag.dot.5.svg" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.6.svg" alt='image of a DAG; each one is one more level than the last'>
Note that UTXOs are not only in the bottom level



##  
<img src="images/mining/dag/dag.dot.7.svg" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.8.svg" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.9.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.10.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.11.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.12.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.13.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.14.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.15.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.16.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.17.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.18.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.19.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>



##  
<img src="images/mining/dag/dag.dot.20.svg" class="stretch" alt='image of a DAG; each one is one more level than the last'>


## Realism
- Most (all?) cryptocurrencies are mined by the developers for the first many blocks
  - Ethereum's first non-mining transaction was in block [46,147](https://etherscan.io/block/46147)
...
## DAG size <!-- change each semester -->
- You can see DAG sizes at [https://minerstat.com/dag-size-calculator](https://minerstat.com/dag-size-calculator)
...
<img src="../slides/images/logos/btc-coin-symbol.svg" class="cclogosubtitle" alt='BTC symbol'>
<img src="../slides/images/logos/btg-coin-symbol.svg" class="cclogosubtitle" alt='BTG symbol'>
<img src="../slides/images/logos/erg-coin-symbol.svg" class="cclogosubtitle" alt='ERG symbol'>
<img src="../slides/images/logos/eth-coin-symbol.svg" class="cclogosubtitle" alt='ETH symbol'>
<img src="../slides/images/logos/firo-coin-symbol.svg" class="cclogosubtitle" alt='FIRO symbol'>
<img src="../slides/images/logos/rvn-coin-symbol.svg" class="cclogosubtitle" alt='RVN symbol'>
<img src="../slides/images/logos/xlm-coin-symbol.svg" class="cclogosubtitle" alt='XLM symbol'>
...
<img src='images/bitcoin/attack/attack.dot.1.svg' style='width:110px;height:auto;max-height:unset;margin-top:0' class='right-float-img' alt='image of a block on a blockchain'>
...
<img src='images/bitcoin/attack/attack.dot.2.svg'  class='right-float-img' style='width:110px;height:auto;max-height:unset;margin-top:0' alt='image of a block on a blockchain with a single child'>
...
<img src='images/bitcoin/attack/attack.dot.3.svg' style='width:110px;height:auto;max-height:unset;margin-top:0' class='right-float-img' alt='image of a block on a blockchain with four children in a line'>
...
<img src='images/bitcoin/attack/attack.dot.4.svg' style='width:220px;height:auto;max-height:unset;margin-top:0' class='right-float-img' alt='image of a block on a blockchain with four children in a line, and also another child'>
...
<img src='images/bitcoin/attack/attack.dot.5.svg' style='width:220px;height:auto;max-height:unset;margin-top:0' class='right-float-img' alt='image of a block on a blockchain with four children in a line, and the other child now has 5 children'>
...
- Consider the [![rvn logo](../slides/images/logos/rvn-coin-symbol.svg){alt='rvn logo'}](https://coinmarketcap.com/currencies/ravencoin/){class="cclogo-slide" alt='RVN logo'} Ravencoin [pool from 2miners](https://2miners.com/rvn-mining-pool)
  - See what percentage they have now by dividing the pool hash rate by the network hash rate
  - On June 27th, 2021, they had over 51% ([source](https://www.reddit.com/r/Ravencoin/comments/o96qox/2miners_is_showing_over_51_on_miningpoolstats/))
    - Due to a purchase of hashpower from (likely) Nicehash
- [![firo logo](../slides/images/logos/firo-coin-symbol.svg){alt='firo logo'}](https://coinmarketcap.com/currencies/firo/){class="cclogo-slide" alt='FIRO logo'} Firo had one in January 2021
- [![btg logo](../slides/images/logos/btg-coin-symbol.svg){alt='btg logo'}](https://coinmarketcap.com/currencies/bitcoin-gold/){class="cclogo-slide" alt='BTG logo'} Bitcoin Gold had one in May 2018 and January 2020
...
<img src="images/bitcoin/attack/attack.dot.6.svg" style="height:auto;max-height:unset;margin-top:0" class='right-float-img' alt='image of a block on a blockchain with two children; each of which has one child of its own'>
...
- Number of confirmations typically required (via [2miners](https://2miners.com)):
  - [![xlm logo](../slides/images/logos/xlm-coin-symbol.svg){alt='xlm logo'}](https://coinmarketcap.com/currencies/stellar/){class="cclogo-slide" alt='XLM logo'} XLM: 0 blocks (5 seconds) (TXNs are irreversible)
  - [![btc logo](../slides/images/logos/btc-coin-symbol.svg){alt='btc logo'}](https://coinmarketcap.com/currencies/bitcoin/){class="cclogo-slide" alt='BTC logo'} BTC: 6 blocks (about 1 hour) for "standard" transactions
  - [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide" alt='ETH logo'} ETH: 240 blocks (about 1 hour) (maybe different w/PoS)
  - [![rvn logo](../slides/images/logos/rvn-coin-symbol.svg){alt='rvn logo'}](https://coinmarketcap.com/currencies/ravencoin/){class="cclogo-slide"} RVN: 100 blocks (about 2 hours)
  - [![firo logo](../slides/images/logos/firo-coin-symbol.svg){alt='firo logo'}](https://coinmarketcap.com/currencies/firo/){class="cclogo-slide"} FIRO: 100 blocks (about 9 hours)
  - [![btc logo](../slides/images/logos/btc-coin-symbol.svg){alt='btc logo'}](https://coinmarketcap.com/currencies/bitcoin/){class="cclogo-slide"} BTC: 60 blocks (about 10 hours) for "large" transactions
  - [![erg logo](../slides/images/logos/erg-coin-symbol.svg){alt='erg logo'}](https://coinmarketcap.com/currencies/ergo/){class="cclogo-slide"} ERG: 720 blocks (about 1 day)
...
<img src="images/logos/btg-coin-symbol.svg" class="cclogosubtitle">
...
<img src="images/logos/nmc-coin-symbol.svg" class="cclogosubtitle">
<img src="images/logos/rvn-coin-symbol.svg" class="cclogosubtitle">


## Cryptocurrency "Forks"
- There are four different meanings of "fork" with regard to cryptocurrencies
  1. Uncle / orphan / ommer blocks
  2. Soft fork 
  3. Hard fork
  4. Source code fork
...
<img src="images/mining/orphan/orphan.dot.1.svg" style="width:auto;height:200px;padding-left:86px;padding-right:86px" class='right-float-img'>
...
<img src="images/mining/orphan/orphan.dot.2.svg" style="width:150px !important;height:auto;max-height:unset" class='right-float-img'>
...
<img src="images/mining/orphan/orphan.dot.3.svg" style="width:250px !important;height:auto;max-height:unset" class='right-float-img'>
...
<img src="images/mining/orphan/orphan.dot.4.svg" style="width:250px !important;height:auto;max-height:unset" class='right-float-img'>
...
<img src="images/mining/orphan/orphan.dot.5.svg" style="width:250px !important;height:auto;max-height:unset" class='right-float-img'>
...
<img src="images/mining/orphan/orphan.dot.6.svg" style="width:250px !important;height:auto;max-height:unset" class='right-float-img'>
...
<img src="https://miro.medium.com/max/1400/1*FBNBdZKQOI2FfJMRBto5pQ.jpeg" style="padding:0;background:transparent;width:400px;height:auto" class='right-float-img'>
...
- Non-token crypto-currencies usually have a group of developers
  ...
  - Bitcoin via [BIPs](https://github.com/bitcoin/bips/), Ethereum via [EIPs](https://github.com/ethereum/EIPs/), etc.
...
- Example: Bitcoin's [Segregated Witness](bitcoin.html#/segwit) from 2015
  - Defined in [BIP 141](https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki)
...
![](https://upload.wikimedia.org/wikipedia/commons/3/39/Three_tines.jpg){class='right-float-img no-border' style="width:400px;height:auto" alt='picture of a tableware fork'}

- A change is proposed that some users do *NOT* want to adopt
  - Or that is not backwards-compatible, so some users choose one way and some the other way



<!-- .slide: class="cclogo-slide" -->
## [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide"} Ethereum & <br> [![etc logo](../slides/images/logos/etc-coin-symbol.svg){alt='etc logo'}](https://coinmarketcap.com/currencies/ethereum-classic/){class="cclogo-slide"} Ethereum Classic
...
- June 2016: A well-known Ethereum organization had a programming bug that allowed millions to be drained from the account
  ...
  - We'll see this error in detail in [Blockchain Applications](applications.html#/)
- The fix: roll back the blockchain to before the hack to fix it
- This [split the users](applications.html#/etcsplit); some didn't want the fix
- [![etc logo](../slides/images/logos/etc-coin-symbol.svg){alt='etc logo'}](https://coinmarketcap.com/currencies/ethereum-classic/){class="cclogo-slide"} Ethereum Classic is the blockchain *without* the fix
- [![eth logo](../slides/images/logos/eth-coin-symbol.svg){alt='eth logo'}](https://coinmarketcap.com/currencies/ethereum/){class="cclogo-slide"} Ethereum is the blockchain *with* the fix



<!-- .slide: class="cclogo-slide" -->
## [![btg logo](../slides/images/logos/btg-coin-symbol.svg){alt='btg logo'}](https://coinmarketcap.com/currencies/bitcoin-gold/){class="cclogo-slide"} Bitcoin Gold
...
- The [![rvn logo](../slides/images/logos/rvn-coin-symbol.svg){alt='rvn logo'}](https://coinmarketcap.com/currencies/ravencoin/){class="cclogo-slide"} [Ravencoin (RVN) source code](https://github.com/RavenProject/Ravencoin) is based off of the [Bitcoin source code](https://github.com/bitcoin/bitcoin); created in 2018
- Changes:
  - 1000x as many coins
...
<!-- .slide: class="cclogo-slide" -->
## [![nmc logo](../slides/images/logos/nmc-coin-symbol.svg){alt='nmc logo'}](https://coinmarketcap.com/currencies/namecoin/){class="cclogo-slide"} Namecoin (2011)
...
- .bit domains are not widely supported
  - Not sanction by [ICANN](https://en.wikipedia.org/wiki/ICANN), not supported by [OpenNIC](https://en.wikipedia.org/wiki/OpenNIC)
