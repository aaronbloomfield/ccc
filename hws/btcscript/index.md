...
1. Create an account at [https://accounts.blockcypher.com/](https://accounts.blockcypher.com/), which will allow you to get an API token.  Your token will be a hex number such as 0123456789abcdef0123456789abcdef.  Save this token in the `blockcypher_api_token` field in scripts.py.
2. You will need to generate a BCY key pair.  Run `python3 bitcoinctl.py bcy_key` -- there are four values provided, and you want the "private" and the "address" fields.  You are welcome to save the others, but we will not need them in this assignment.  While these keys are not valid on the main Bitcoin test network -- the have a different value for the [version byte](../../slides/bitcoin.html#/btcaddress) in the invoice address -- you will need them throughout this assignment.
    - Save both the BCY private key and the BCY address into the scripts.py file into the `private_key_str` and `invoice_address` fields, respectively.
    - While you are at it, you will need three more accounts for part 3 (multi-sig) -- you are welcome to create them now.  They go into the six fields at the start of the part 3 section (`alice_private_key_str,` alice_invoice_address_str`, and similarly for bob and charlie's keys).
3. Only two invoice addresses need funds -- the primary one (the first one you generated), and Bob's; Alice and Charlie's accounts do not need funds.  To generate the funds for your primary account, run `python3 bitcoinctl.py fund`.  We will hold off, for now, on funding Bob's account.
    - If you end up having to fund your account again, put the next funding transaction hash as the next element in that list, etc.
    - The above command will return a transaction hash; save that as the first (and, currently, only) value in `txid_funding_list`.  If you run `python3 bitcoinctl.py urls` it will display the full URL that you can use to view that funding transaction.
4. The faucet transaction paid to the invoice address via only one UTXO, and we would like multiple UTXO indices to use -- this way we can use one per question part, and we have a few extra for if and when something ends up not working correctly.
    - We are going to split the incoming UTXO into multiple smaller UTXOs.  Each of the smaller UTXOs will need to about 1/10th of the BCY received from the faucet.
        - Look at the section of scripts.py that deals with splitting coins.  The default values there may need to be changed
            - The `txid_split` is the particular transaction hash that you are splitting, which should be in the `txid_funding_list` list.  It is currently set to the first (aka 0th) and only element in `txid_funding_list`.  If you have to fund your account again, change the index for your more recent funding transaction hash.
            - The `split_amount_to_split` is how much is in the incoming UTXO; look this up on [https://live.blockcypher.com](https://live.blockcypher.com) to get the correct amount.  It should be 0.001 BCY.
            - The `split_amount_after_split` is how much you want in each UTXO after the split -- it should about 1/10th of the amount in the incoming UTXO.
        ...
        - **DETERMINE YOUR UTXO INDEX**: this is provided to the scripts via the second command-line parameter, and you must set it for the particular UTXO index for the transaction where that you are splitting.  Some faucets pay themselves in UTXO index 0, and then pay out the requested invoice address in index 1.
            - Check the transaction -- via the URL from above (run `python3 bitcoinctl.py urls` for the URL list) -- that gave you the coins, and make sure you specify the right UTXO index (via the second command-line parameter).  If you get an error such as "witness script detected in tx without witness data", then it probably means your UTXO index is wrong.  When viewing the transaction, the UTXOs are shown in ascending numerical order (indexed from 0).
...
The UTXO indices that you created when you split your BCY are paid to a standard [P2PKH transaction](../../slides/bitcoin.html#/p2pkh).  Your task is to redeem them by writing the appropriate scripts (sigscript (input) and pubKey script (output)) to redeem the coins from one of the UTXOs.  It should be paid back to the designated return address -- bitcoinctl.py uses the `tbtc_return_address` variable, defined at the top of the `scripts.py` file, as the receiver of this transaction.
...
- The `P2PKH_scriptPubKey()` function defines the pubKey script (aka output script).  This was discussed in lecture in the [P2PKH transaction](../../slides/bitcoin.html#/p2pkh) slides.  This script creates a new UTXO, payable to the BCY return address, that is also a P2PKH script.  The parameter is of type `P2PKHBitcoinAddress`, which is what the `P2PKHBitcoinAddress.from_pubkey(public_key)` call (shown above) returns; a variable of this type can be put directly into a script.
- Set the UTXO index to spend via the second command line parameter.  Recall that output indices start from 0, not 1.  Be sure to pick an unspent index!  If you have to run this multiple times, you may have to change this value to an unspent index.
...
- To fund the account, we use "fund_bob": `python bitcoinctl.py fund_bob`; the transaction hash from this should be stored in `txid_bob_bcy_funding`
- To split the account, we use "split_bob": `python bitcoinctl.py split_bob`; the transaction has from this should be stored in `txid_bob_bcy_split`
...
- Ensure that the final balance of the BCY wallet (from `my_invoice_address_str`) is zero
