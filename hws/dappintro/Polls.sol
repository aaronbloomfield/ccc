...
contract Polls is IPolls {
	...
	// struct Choice is defined in IPolls.sol, so we can't re-define
	...
	// The following two events are defined in IPolls.sol, so we can't
	...
	// IPolls.sol, we have to put the 'override' keyword here.
	...
	// returned from the choices() function defined in IPolls.sol
	...
	// implementing the one specified in IPolls.sol, we have to put
	...
	// function of the same name from IPolls.sol, so we put
	// the 'override' keyword here.
	...
	// of the same name from IPolls.sol, so we put the 'override'
	// keyword here.
	...
	// the object at all.  As it is not defined in IPolls.sol, we do
	...
	// not defined in IPolls.sol, we do not need the 'override'
	// keyword here.
	function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
		return interfaceId == type(IPolls).interfaceId || interfaceId == 0x01ffc9a7;
	}
