...
- ether price: $100
- TCC price: $10
- max ether to trade: 10
- max TCC to trade: 100

Each run assumes the state from the previous run.  The trades are:

- Before first run: 10 ether, 0 TCC, holdings of 1000.00 USD
- Run 1: starting with 11 ether and no TCC, and with all the DEXes enabled in the config file, it should send 10 ether (the max allowed) to the d20 DEX
    - That will result in a receipt of about 181 TC
    - Output line: `Exchanged 10.0000 ETH for 180.9091 TC; fees: 0.16 USD; prices: ETH 100.00 USD, TC: 10.00 USD; holdings: 1908.94 USD`
        - Depending on how you computed fees, there is some variance ($0.05) allowed in the fee amount; likewise, the final holdings can vary by that same amount ($0.05)
    - Balances after: 0.998503009999999969 ether, TC balance of 180.9090909092
- Run 2: with only the lower 3 DEXes enabled (d4, d6, and d8), 10 ether, and 100 TC, it should send 100 TC (the max allowed) to the d4 DEX
    - That will result in a gain of about 11 ether
    - Output line: `Exchanged 100.0000 TC for 19.8987 ETH; fees: 0.15 USD; prices: ETH 100.00 USD, TC: 10.00 USD; holdings: 3089.78 USD`
        - Depending on how you computed fees, there is some variance ($0.05) allowed in the fee amount; likewise, the final holdings can vary by that same amount ($0.05)
    - Balances after: 30.897804419999996384 ether, TC balance of 0
- Run 3: with only the d10 DEX enabled in the config file, it should not find a profitable trade
